...
function validate(form: FormState): FormErrors {
    ...
    if (form.user_base_salary) {
        const n = Number(form.user_base_salary);
        if (!Number.isFinite(n) || n < 0) errors.user_base_salary = "เงินเดือนพื้นฐานต้องเป็นตัวเลขไม่ติดลบ";
    }
    ...
    useEffect(() => {
        ...
        startTransition(async () => {
            ...
            setForm({
                ...
                user_base_salary: data.user_base_salary != null ? String(data.user_base_salary) : "",
            });
    ...
    return (
        <div className="p-4 sm:p-6 max-w-2xl mx-auto">
            ...
            <Form cols={2} onSubmit={handleSubmit} className="bg-white shadow-sm border-gray-100 rounded-xl">
                ...
                <div className="flex flex-col gap-1 col-span-2">
                    <label className="text-sm font-medium text-gray-700">เงินเดือนพื้นฐาน (บาท)</label>
                    <Input type="number" min={0} step="0.01" name="user_base_salary" value={form.user_base_salary} onChange={handleChange}
                        placeholder="เว้นว่างถ้าไม่ใช่พนักงานที่รับเงินเดือนผ่านระบบนี้" error={!!errors.user_base_salary} />
                    {errors.user_base_salary && <p className="text-xs text-red-500">{errors.user_base_salary}</p>}
                    <p className="text-xs text-gray-400">ใช้ auto-fill ตอนเปิดโมดัล &quot;จ่ายเงินเดือนพนักงานแบบชุด&quot; ที่หน้าเงินเดือนพนักงาน ไม่ต้องพิมพ์ใหม่ทุกเดือน</p>
                </div>
