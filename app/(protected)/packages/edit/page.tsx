...
type PackageDetail = {
    ...
    items: { pki_product_id: string; prod_name: string; prod_price: number; prod_is_free: boolean }[];
};

async function fetchPackage(id: string) {
    const res = await fetch(`${api}/packages/${id}`, { headers: authHeader() });
    if (!res.ok) return null;
    return res.json() as Promise<PackageDetail>;
}

async function submitUpdatePackage(id: string, body: Record<string, unknown>) {
...
export default function EditPackagePage() {
    ...
    const [status, setStatus] = useState<PackageDetail["pkg_status"]>("draft");
    ...
    useEffect(() => {
        ...
        fetchPackage(id).then((data) => {
            ...
            setName(data.pkg_name);
            setDescription(data.pkg_description ?? "");
            setPrice(String(data.pkg_price));
            setStatus(data.pkg_status);
            setSelected(new Map(data.items.map((it) => [
                it.pki_product_id,
                { prod_id: it.pki_product_id, prod_name: it.prod_name, prod_price: it.prod_price, prod_is_free: it.prod_is_free, prod_cover_url: null },
            ])));
            if (data.pkg_cover_url) setCurrentCover(`${SERVER_BASE}${data.pkg_cover_url}`);
            setLoaded(true);
        });
    }, [id]);
    ...
    if (!id || notFound) return <p className="p-6 text-gray-500">ไม่พบแพ็กเกจนี้</p>;
    if (!loaded) return <p className="p-6 text-gray-400">กำลังโหลด...</p>;

    return (
        <div className="p-4 sm:p-6 max-w-2xl mx-auto">
            <h1 className="text-xl sm:text-2xl font-bold text-gray-800 mb-6">แก้ไขแพ็กเกจรวมชุด</h1>
            ...
            <Form cols={1} onSubmit={handleSubmit} className="bg-white shadow-sm border-gray-100 rounded-xl">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <Input value={name} onChange={handleNameChange} error={!!errors.name} />
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <select
                        value={status}
                        onChange={(e) => setStatus(e.target.value as PackageDetail["pkg_status"])}
                        className="px-4 py-2 border rounded focus:outline-none focus:ring-2 border-gray-300 focus:border-blue-400 focus:ring-blue-500/20"
                    >
