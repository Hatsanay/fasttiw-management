...
async function fetchExpense(id: string) {
    const res = await fetch(`${api}/expenses/${id}`, { headers: authHeader() });
...
export default function EditExpensePage() {
    ...
    useEffect(() => {
        ...
        startTransition(async () => {
            ...
            setCategoryId(data.exp_category_id ?? "");
            setProductId(data.exp_product_id ?? "");
            setAmount(String(data.exp_amount ?? ""));
            setNote(data.exp_note ?? "");
            setDate((data.exp_date ?? "").slice(0, 10));
        });
    }, [id]);
    ...
    if (!id || notFound) return <p className="p-6 text-gray-500">ไม่พบรายการค่าใช้จ่ายนี้</p>;

    return (
        <div className="p-4 sm:p-6 max-w-2xl mx-auto">
            <h1 className="text-xl sm:text-2xl font-bold text-gray-800 mb-6">แก้ไขค่าใช้จ่าย</h1>
