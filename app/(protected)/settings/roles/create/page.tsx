...
type FormErrors = { role_name?: string };

function validate(roleName: string): FormErrors {
    const errors: FormErrors = {};
    if (!roleName.trim())              errors.role_name = "กรุณากรอกชื่อสิทธิ์";
    else if (roleName.trim().length < 2) errors.role_name = "ชื่อต้องมีอย่างน้อย 2 ตัวอักษร";
    return errors;
}
...
export default function CreateRolePage() {
    ...
    function handleNameChange(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        if (errors.role_name) setErrors((prev) => ({ ...prev, role_name: undefined }));
    }
    ...
    function toggleBit(index: number) {
        setChecked((prev) => {
            ...
            // ติ๊ก child → ติ๊ก bit แรกของกลุ่ม (view) ด้วยอัตโนมัติ
            if (next[index]) {
    ...
    return (
        <div className="p-4 sm:p-6 max-w-2xl mx-auto">
            <h1 className="text-xl sm:text-2xl font-bold text-gray-800 mb-6">สร้างสิทธิ์</h1>
