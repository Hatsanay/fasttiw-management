...
type FormErrors = { dep_name?: string };

function validate(depName: string): FormErrors {
    const errors: FormErrors = {};
    if (!depName.trim())              errors.dep_name = "กรุณากรอกชื่อแผนก";
    else if (depName.trim().length < 2) errors.dep_name = "ชื่อต้องมีอย่างน้อย 2 ตัวอักษร";
    return errors;
}
...
export default function CreateDepartmentPage() {
    ...
    function handleNameChange(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        if (errors.dep_name) setErrors((prev) => ({ ...prev, dep_name: undefined }));
    }
        ...
        <div className="p-4 sm:p-6 max-w-2xl mx-auto">
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
                <div>
                    ...
                    />
                    {errors.dep_name && <p className="text-xs text-red-500 mt-1">{errors.dep_name}</p>}
                </div>
